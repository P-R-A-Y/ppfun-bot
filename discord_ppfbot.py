...
headers = {
    "Host": "pixelplanet.fun","User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:102.0) Gecko/20100101 Firefox/102.0","Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8","Accept-Language": "ru-RU,ru;q=0.8,en-US;q=0.5,en;q=0.3","Accept-Encoding": "gzip, deflate, br","Connection": "keep-alive","Upgrade-Insecure-Requests": "1","Sec-Fetch-Dest": "document","Sec-Fetch-Mode": "navigate","Sec-Fetch-Site": "none","Sec-Fetch-User": "?1","TE": "trailers"
}
defaultheaders = {
    "Host": "pixelplanet.fun","User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:102.0) Gecko/20100101 Firefox/102.0","Accept": "*/*","Accept-Language": "ru-RU,ru;q=0.8,en-US;q=0.5,en;q=0.3","Accept-Encoding": "gzip, deflate, br","Connection": "keep-alive","Referer": "https://pixelplanet.fun/","Sec-Fetch-Dest": "script","Sec-Fetch-Mode": "no-cors","Sec-Fetch-Site": "same-origin","TE": "trailers"
}
websocketheaders = {
    "Host": "pixelplanet.fun","User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:102.0) Gecko/20100101 Firefox/102.0","Accept": "*/*","Accept-Language": "ru-RU,ru;q=0.8,en-US;q=0.5,en;q=0.3","Accept-Encoding": "gzip, deflate, br","Sec-WebSocket-Version": "13","Origin": "https://pixelplanet.fun","Sec-WebSocket-Extensions": "permessage-deflate","Connection": "keep-alive, Upgrade","Sec-Fetch-Dest": "websocket","Sec-Fetch-Mode": "websocket","Sec-Fetch-Site": "same-origin","Pragma": "no-cache","Cache-Control": "no-cache","Upgrade": "websocket"
}
captchaheaders = {
    "Host": "pixelplanet.fun","User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:102.0) Gecko/20100101 Firefox/102.0","Accept": "*/*","Accept-Language": "ru-RU,ru;q=0.8,en-US;q=0.5,en;q=0.3","Accept-Encoding": "gzip, deflate, br","Referer": "https://pixelplanet.fun/","Content-Type": "application/json","Origin": "https://pixelplanet.fun","Alt-Used": "pixelplanet.fun","Connection": "keep-alive","Sec-Fetch-Dest": "empty","Sec-Fetch-Mode": "cors","Sec-Fetch-Site": "same-origin"
}

defaultchannel = 858076704823181366
...
class Bot():
    ...
    async def onCaptcha(self):
        if self.proxy is None:
            c = self.session.get("https://pixelplanet.fun/captcha.svg",headers=defaultheaders)
        else:
            c = self.session.get("https://pixelplanet.fun/captcha.svg",headers=defaultheaders,proxies=self.proxy)
    ...
    def sendCaptcha(self, id, text):
        ...
        if self.proxy is None:
            r = self.session.post("https://pixelplanet.fun/api/captcha",headers=h,json=data)
        else:
            r = self.session.post("https://pixelplanet.fun/api/captcha",headers=h,json=data,proxies=self.proxy)
    ...
    def get_chunk(self, x, y):
        if self.proxy is None:
            data = self.session.get(f'https://pixelplanet.fun/chunks/{targetCanvas}/{x}/{y}.bmp',headers=defaultheaders).content
        else:
            data = self.session.get(f'https://pixelplanet.fun/chunks/{targetCanvas}/{x}/{y}.bmp',headers=defaultheaders, proxies=self.proxy).content
    ...
    def connect(self):
        ...
        if self.proxy is None:
            ppf = self.session.get("https://pixelplanet.fun/#d,-177,-19,17", headers = headers)
        else:
            ppf = self.session.get("https://pixelplanet.fun/#d,-177,-19,17", headers = headers, proxies=self.proxy)
        ...
        if self.proxy is None:
            vendor = self.session.get(f"https://pixelplanet.fun/assets/{vnd}",headers=defaultheaders)
            client = self.session.get(f"https://pixelplanet.fun/assets/{clnt}",headers=defaultheaders)
        else:
            vendor = self.session.get(f"https://pixelplanet.fun/assets/{vnd}",headers=defaultheaders,proxies=self.proxy)
            client = self.session.get(f"https://pixelplanet.fun/assets/{clnt}",headers=defaultheaders,proxies=self.proxy)
        #with open(f"{dir}cl.txt", "wb") as f:
        #    f.write(client.content)
        #    f.close()
        if self.proxy is None:
            self.me = self.session.get('https://pixelplanet.fun/api/me',headers=defaultheaders).json()
        else:
            self.me = self.session.get('https://pixelplanet.fun/api/me',headers=defaultheaders,proxies=self.proxy).json()
        ...
        if self.proxy is None:
            self.ws = websocket.create_connection("wss://pixelplanet.fun:443/ws",header = websocketheaders)
        else:
            self.ws = websocket.create_connection("wss://pixelplanet.fun:443/ws",header = websocketheaders,http_proxy_host=str(self.proxy["http"]).split("//")[1].split(":")[0],http_proxy_port=str(self.proxy["http"]).split("//")[1].split(":")[1])
...
@client.event
async def on_ready():
    print("token:"+token+" started"+" bot name: "+client.user.name)
